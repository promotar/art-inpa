...
class OldSiteImporter
{
    ...
    private function importPost(array $wpPost, Command $command): void
    {
        ...
        try {
            ...
            $payload = [
                ...
                'seo_score' => $this->seoScore($seo, $content, $categoryIds, $tagIds, $featured),
    ...
    private function seoScore(array $seo, string $content, array $categoryIds, array $tagIds, ?array $featured): int
    {
        $score = 25;
        $score += $seo['seo_title'] !== '' ? 15 : 0;
        $score += $seo['seo_description'] !== '' ? 15 : 0;
        $score += $seo['focus_keyword'] !== '' ? 10 : 0;
        $score += str_word_count($this->plainText($content)) >= 300 ? 10 : 0;
        $score += ! empty($categoryIds) ? 10 : 0;
        $score += ! empty($tagIds) ? 5 : 0;
        $score += $featured ? 10 : 0;
